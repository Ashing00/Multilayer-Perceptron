...
#訓練樣本+
nn.fit(train_data, train_label, print_progress=True)
train_pred = nn.predict(train_data)
acc = np.sum(train_label == train_pred, axis=0) / train_data.shape[0]
print('Training accuracy: %.2f%%' % (acc * 100))

#預測測試樣本1
train_pred2 = nn.predict(X_train)
acc = np.sum(y_train == train_pred2, axis=0) / X_train.shape[0]
print('test1 accuracy: %.2f%%' % (acc * 100))
#預測測試樣本2
train_pred3 = nn.predict(X_test)
acc = np.sum(y_test == train_pred3, axis=0) / X_test.shape[0]
print('test2 accuracy: %.2f%%' % (acc * 100))


#預測測試樣本+
test_pred = nn.predict(test_data)
print("test_pred=",test_pred)

#儲存預測測試樣本結果
saveResult(test_pred) 
